  ...
  render () {
    ...
    return (
      <Link href={path} as={as}>
        <a className={classNames(className, { 'has-opacity-medium': pathname === path })} style={{ backgroundColor: `${backgroundColor || 'transparent'}`, color: `${color || '#FFFFFF'}` }} dangerouslySetInnerHTML={{ __html: label }}>
      </Link>
    )
  }
}
